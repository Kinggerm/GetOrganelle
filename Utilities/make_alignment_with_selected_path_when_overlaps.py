...
def require_commands():
    global options
    usage = 'python '+str(os.path.basename(__file__))+' -f input.fasta'
...
def check_similarity_and_continuity(string1, string2):
    global options
...
def main():
    ...
    while i < len(first_seq):
        if first_seq[i] == '?':
            ...
            if check_similarity_and_continuity(first_seq[this_start-this_overlap_len:this_start], first_seq[this_end+1:this_end+this_overlap_len+1]):
    ...
    write_fasta(options.in_fasta_file+'.contig_alignments.fasta', new_fasta_matrix, False)
